...
def login(usr, pwd):
    ...
    while not ret.get('state', False):
        ...
        if code == 3425 or code == 1000:
            ...
            data.update({'TPL_checkcode': check_code,
                         'need_check_code': 'true'})
            ret = login_post(data)
        else:
            sys.exit('ERROR. code: {}, message:{}'.format(
                code, ret.get('message', '')))
                ...
                for n in node:
                    try:
                        ...
                        if True:
                            ...
                            # 宝贝价格
                            bb['price'] = float(
                                n.find('td', class_='price').attrs['title'])
                            # 宝贝数量
                            bb['quantity'] = int(
                                n.find('td', class_='quantity').attrs['title'])
                            bb['is_goods'] = True
                        baobei.append(bb)
                        # 尝试获取实付款
                        # 实付款所在的节点可能跨越多个tr的td
...
def output(orders, start_date, end_date):
    ...
    if invaild_order_count:
        print('{:<9} {} (退货或取消等, 不在上述订单之内)'.format(
            '无效订单:', invaild_order_count))
...
def ouput_orders(orders):
    ...
    for order in orders:
        print(' {:-^20}'.format('-'))
        print(
            ' * 订单号: {orderid} 实付款: {amount} 店铺: {shop_name} 时间: {date:%Y-%m-%d %H:%M}'.format(**order))
